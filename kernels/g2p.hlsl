...
cbuffer Params : register(b0)
{
  ...
  float3 origin; float h;
}

[numthreads(256,1,1)]
void cs_g2p(uint3 tid : SV_DispatchThreadID)
{
  uint i = tid.x;
  ...
  for (int dx=0; dx<3; ++dx) {
    ...
    if (gi.x < 0 || gi.y < 0 || gi.z < 0 || gi.x >= (int)nx || gi.y >= (int)ny || gi.z >= (int)nz) continue;
    uint idx = (gi.z*ny + gi.y)*nx + gi.x;
    float m = g_mass[idx]; if (m <= 0.0) continue;
    float w = wx[dx]*wy[dy]*wz[dz];
    float3 vi = float3(g_vel[3*idx+0], g_vel[3*idx+1], g_vel[3*idx+2]);
    v += w * vi;
    float3 xrel = (float3(gi) - gp);
    Cacc[0][0] += w * vi.x * xrel.x; Cacc[0][1] += w * vi.x * xrel.y; Cacc[0][2] += w * vi.x * xrel.z;
    Cacc[1][0] += w * vi.y * xrel.x; Cacc[1][1] += w * vi.y * xrel.y; Cacc[1][2] += w * vi.y * xrel.z;
    Cacc[2][0] += w * vi.z * xrel.x; Cacc[2][1] += w * vi.z * xrel.y; Cacc[2][2] += w * vi.z * xrel.z;
  }
  float s = 4.0 / (h*h);
  ...
  uint ci = i*9;
  ParticlesRW.C[ci+0] = s*Cacc[0][0]; ParticlesRW.C[ci+1] = s*Cacc[0][1]; ParticlesRW.C[ci+2] = s*Cacc[0][2];
  ParticlesRW.C[ci+3] = s*Cacc[1][0]; ParticlesRW.C[ci+4] = s*Cacc[1][1]; ParticlesRW.C[ci+5] = s*Cacc[1][2];
  ParticlesRW.C[ci+6] = s*Cacc[2][0]; ParticlesRW.C[ci+7] = s*Cacc[2][1]; ParticlesRW.C[ci+8] = s*Cacc[2][2];
}
