...
// \credits See docs: ALGORITHMS.md (P2G/G2P Transfers, APIC/MLS) and ARCHITECTURE.md (Execution Graph)
...
[numthreads(256,1,1)]
void cs_p2g(uint3 tid : SV_DispatchThreadID)
{
  uint i = tid.x;
  ...
  if (Particles.C.GetDimensions() > 0) {
    uint ci = i*9;
    Cp[0][0] = Particles.C[ci+0]; Cp[0][1] = Particles.C[ci+1]; Cp[0][2] = Particles.C[ci+2];
    Cp[1][0] = Particles.C[ci+3]; Cp[1][1] = Particles.C[ci+4]; Cp[1][2] = Particles.C[ci+5];
    Cp[2][0] = Particles.C[ci+6]; Cp[2][1] = Particles.C[ci+7]; Cp[2][2] = Particles.C[ci+8];
  }
