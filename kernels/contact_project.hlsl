...
cbuffer Params : register(b0)
{
  ...
  float mu; float kappa_n; // friction and compliance
}

[numthreads(256,1,1)]
void cs_contact(uint3 tid : SV_DispatchThreadID)
{
  uint idx = tid.x;
  uint total = nx*ny*nz;
  if (idx >= total) return;
  float m = g_mass[idx]; if (m <= 0.0) return;
  ...
  if (phi < 0.0) {
    ...
    if (vn < -allowance) vn = -allowance;
  }
  float vt_len = length(vt);
  if (vt_len > 1e-12) {
    float max_t = mu * max(0.0, vn);
    if (vt_len > max_t) vt *= (max_t / vt_len);
  }
  v = vn*n + vt;
  g_vel[3*idx+0] = v.x;
  g_vel[3*idx+1] = v.y;
