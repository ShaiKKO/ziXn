...
int main(){
    ...
    for (uint32_t seed=1; seed<=5; ++seed){
        ...
        for (uint32_t y=0;y<H;++y){ for (uint32_t x=1;x<W;++x){ float r=(float)(lcg(rng)&0xFFFF)/65535.0f; B[y*W + x] = r; } }
        ...
        float d0 = zx_lod_border_consistency_check(A1.data(), W/2,H/2,W/2, B1.data(), W/2,H/2,W/2, 0);
        assert(d0 < 1e-3f);
        ...
        float d1 = zx_lod_border_consistency_check(A2.data(), W/4,H/4,W/4, B2.data(), W/4,H/4,W/4, 0);
        assert(d1 < 1e-3f);
    }
    return 0;
}
