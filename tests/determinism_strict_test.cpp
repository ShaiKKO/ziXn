...
static bool files_equal(const char* a, const char* b){
    FILE* fa = std::fopen(a, "rb"); if(!fa) return false;
    FILE* fb = std::fopen(b, "rb"); if(!fb){ std::fclose(fa); return false; }
    int ca=0, cb=0; bool ok=true;
    while (ok){ ca = std::fgetc(fa); cb = std::fgetc(fb); if (ca!=cb){ ok=false; break; } if (ca==EOF || cb==EOF) break; }
    std::fclose(fa); std::fclose(fb); return ok;
}

int main(){
    const char* A = "wsl-build/det_a.json";
    const char* B = "wsl-build/det_b.json";
    run_scene_to_json(A);
    run_scene_to_json(B);
