...
static bool file_contains(const char* path, const char* needle)
{
  ...
  size_t rd = std::fread(buf, 1, (size_t) n, f);
  buf[rd]   = '\0';
  std::fclose(f);
  bool ok = std::strstr(buf, needle) != nullptr;
  std::free(buf);
  return ok;
}
...
{
  const char* out = "telem_depth.json";
  // Use fallback 'on' to ensure fallback_* counters are emitted
  char cmd[512];
#if defined(_WIN32)
  std::snprintf(cmd, sizeof(cmd),
                ".\\zx_cli.exe --mode scene --scene %s --deterministic 1 --seed 42 --fallback on "
                "--telemetry-out %s",
                scene, out);
#else
  std::snprintf(cmd, sizeof(cmd),
                "./zx_cli --mode scene --scene %s --deterministic 1 --seed 42 --fallback on "
                "--telemetry-out %s",
                scene, out);
#endif
