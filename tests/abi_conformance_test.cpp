 ...
 * \license Proprietary — Copyright (c) 2025 Colin Macritchie / Ripple Group, LLC. All rights
 * reserved.
 */
...
int main()
{
  ...
  zx_procs Pref{};
  assert(zxGetProcTable(ZX_ABI_VERSION, &Pref) == ZX_OK);
  auto same   = true;
  auto worker = [&]()
  {
    zx_procs Px{};
    zx_status s = zxGetProcTable(ZX_ABI_VERSION, &Px);
    if (s != ZX_OK)
    {
      same = false;
      return;
    }
    if (Px.create_context != Pref.create_context || Px.error_string != Pref.error_string)
      same = false;
  };
