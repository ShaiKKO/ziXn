...
static void print_usage()
{
  ...
  std::printf("  zx_cli --mode anisotropy --mulong <mu0> --mulat <mu1> --vn <vn> --v0 <v_t0> --v1 "
              "<v_t1>\n");
  std::printf("  zx_cli --mode presets [--out <path>]\n");
  std::printf("  zx_cli --mode scene --scene {dambreak|bogging|puddle} [--telemetry-out <path>] "
              "[--deterministic 0|1] [--seed <u64>] [--fallback on|off|auto] "
              "[--fallback-thresholds <tiles> <ms> <enter> <exit> <blend>]\n");
  std::printf(
...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    {
      mu_long = std::atof(argv[++i]);
    }
    else if (std::strcmp(argv[i], "--mulat") == 0 && i + 1 < argc)
    {
      mu_lat = std::atof(argv[++i]);
    }
  ...
  else if (std::strcmp(mode, "anisotropy") == 0)
  {
    ...
    std::printf("anisotropy: mulong=%.3f mulat=%.3f vn=%.3f v0=%.3f v1=%.3f -> "
                "out=(%.3f,%.3f,%.3f) sat=%d\n",
  ...
  else if (std::strcmp(mode, "bench") == 0)
  {
    ...
    for (int y = 0; y < H; ++y)
      for (int x = 0; x < W; ++x)
      {
        A[(size_t) y * W + x] = (float) ((x * 13 + y * 7) & 255) * 0.01f;
      }
    for (int y = 0; y < H; ++y)
    {
      ...
      for (int x = 1; x < W; ++x)
      {
        B[(size_t) y * W + x] = (float) ((x * 3 + y * 5) & 255) * 0.02f;
      }
    }
    int dw = W / 2, dh = H / 2;
    ...
    if (bench_up2)
    {
      ...
      for (int y = 0; y < dh; ++y)
        for (int x = 0; x < dw; ++x)
        {
          D[(size_t) y * dw + x] = (float) (((x * 17 + y * 11) ^ 0x55) & 255) * 0.007f;
        }
      ...
      if (ms_avx2 > 0.0)
        std::printf("speedup (scalar/avx2): %.2fx\n", ms_scalar / ms_avx2);
    ...
    if (std::strcmp(scene_name, "dambreak") == 0)
    {
      ...
      // Residency metrics and optional fallback counters
      {
        ...
        for (uint32_t s = 0; s < p.steps; ++s)
        {
          ...
          {
            if (zx_lod_fallback_update(&fbp, active, 1.0f, &fbs))
              active_frames++;
          }
        }
