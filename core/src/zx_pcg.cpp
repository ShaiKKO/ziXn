...
extern "C"
{
   ...
   * @param apply_prec Optional callback invoked as apply_prec(r, z, user) to compute z := M^{-1} r;
   * may be NULL.
   * @param opts Solver options (must not be NULL): controls max_iters, tol_abs, tol_rel.
   * @param out_final_resid If non-NULL, receives the final residual norm ||b - A x||; may be set to
   * 0 when ||b|| == 0.
   *
   * @return Number of iterations actually performed (<= opts->max_iters). Returns 0 on invalid
