...
void zx_heave_profile_compute(const zx_heave_params* hp, float extent, float* out_y, uint32_t samples)
{
    if (!hp || !out_y || samples == 0) return;
    float dx = hp->grid_dx;
    float half = extent;
