...
void zx_stress_invariants(const float s[9], float* I1, float* J2)
{
    const float I1v = s[0] + s[4] + s[8];
    float p = I1v / 3.0f;
    // deviatoric = s - pI
    float dev[9]; std::memcpy(dev, s, sizeof(dev));
    dev[0] -= p; dev[4] -= p; dev[8] -= p;
    // J2 = 1/2 * dev:dev
    float dd = 0.0f;
    for (int i=0;i<9;++i) dd += dev[i]*dev[i];
    *I1 = I1v;
    *J2 = 0.5f * dd;
}

void zx_dp_return_map(
    ...
    float* delta_gamma_out)
{
    ...
    if (f <= 0.0f && I1 == I1c) {
        ...
        if (delta_gamma_out) *delta_gamma_out = 0.0f;
        return;
    }
    ...
    float sigma_proj[9];
    for (int i=0;i<9;++i) sigma_proj[i] = scale * dev[i];
    sigma_proj[0] += p_new; sigma_proj[4] += p_new; sigma_proj[8] += p_new;
    ...
    N[0] += dp->alpha; N[4] += dp->alpha; N[8] += dp->alpha;
    // deviatoric contribution from trial
    float denom = 2.0f * std::max(sqrtJ2, eps);
    for (int i=0;i<9;++i) N[i] += dev[i] / denom;

    // Plastic multiplier proxy from distance to yield
    float dgamma = std::max(0.0f, f);

    auto dot9 = [](const float* a, const float* b){ float s=0; for(int i=0;i<9;++i) s+=a[i]*b[i]; return s; };
    auto sub9 = [](const float* a, const float* b, float* o){ for(int i=0;i<9;++i) o[i]=a[i]-b[i]; };

    float step = 1.0f;
    float sigma_candidate[9];
    for (int iter=0; iter<10; ++iter) {
        for (int i=0;i<9;++i) sigma_candidate[i] = sigma_trial[i] + step*(sigma_proj[i] - sigma_trial[i]);
        float d_eps_p[9]; for (int i=0;i<9;++i) d_eps_p[i] = (dgamma*step) * N[i];
        float d_sigma[9]; sub9(sigma_candidate, sigma_trial, d_sigma);
        float W = dot9(d_sigma, d_eps_p); // plastic work increment
        if (W >= -1e-6f) {
            std::memcpy(sigma_out, sigma_candidate, sizeof(float)*9);
            if (delta_gamma_out) *delta_gamma_out = dgamma*step;
            return;
        }
        step *= 0.5f; // backtrack
    }
...
static void eig3_sym(const float s[9], float eval[3])
{
    ...
    const float I2 = s[0]*s[4] + s[4]*s[8] + s[8]*s[0] - (s[1]*s[3] + s[2]*s[6] + s[5]*s[7]);
    const float I3 = s[0]*(s[4]*s[8]-s[5]*s[7]) - s[1]*(s[3]*s[8]-s[5]*s[6]) + s[2]*(s[3]*s[7]-s[4]*s[6]);
    // Solve cubic λ^3 - I1 λ^2 + I2 λ - I3 = 0 via Cardano (assuming distinct real roots typical for stresses)
    const float a = -I1; const float b = I2; const float c = -I3;
    ...
    float* eps_v_pl)
{
    ...
    float lam[3]; eig3_sym(sigma_trial, lam);
    // Sort descending
    ...
    float f = tau_max + alpha * (-p) - c_pa; // note p is compression negative if using sign convention
    if (f <= 0.0f) { std::memcpy(sigma_out, sigma_trial, sizeof(float)*9); if (delta_gamma_out) *delta_gamma_out = 0.0f; return; }
    ...
    float lam_new[3]; lam_new[0] = lam[0] - scale * 0.5f * (lam[0] - p);
    ...
    float shift = p_new - p;
    lam_new[0] += shift; lam_new[1] += shift; lam_new[2] += shift;

    // Recompose as diagonal Cauchy (no rotation change for reference path)
    std::memset(sigma_out, 0, sizeof(float)*9);
    sigma_out[0] = lam_new[0]; sigma_out[4] = lam_new[1]; sigma_out[8] = lam_new[2];
    if (eps_v_pl) *eps_v_pl += -gamma * 3.0f * alpha_psi;
