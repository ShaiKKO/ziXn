                     ...
                     float* ZX_RESTRICT m_grid, float* ZX_RESTRICT p_grid)
{
  ...
  for (size_t i = 0; i < N; ++i)
  {
    ...
    const int base_x = static_cast<int>(std::floor(gx - 0.5F));
    const int base_y = static_cast<int>(std::floor(gy - 0.5F));
    const int base_z = static_cast<int>(std::floor(gz - 0.5F));
  ...
  if (deterministic)
  {
    ...
    auto flush = [&]()
    {
      ...
      {
        m_grid[cur] += msum;
        p_grid[3 * cur + 0] += mxsum;
        p_grid[3 * cur + 1] += mysum;
        p_grid[3 * cur + 2] += mzsum;
      }
    };
