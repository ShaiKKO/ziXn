...
extern "C" /**
 * @brief Projects an incoming velocity onto a contact constraint (normal + Coulomb friction).
 *
 ...
 * @param kappa_n Normal stiffness scale used with phi to compute allowed penetration clamp.
 * @param v_out Output velocity after projection (length-3 array), overwritten by the function.
 * @param out_sat If non-null, set to 1 when tangential friction saturates (velocity was clamped), otherwise 0.
 */
ZX_API void ZX_CALL zx_contact_project(const float v_in[3], const float n[3], float mu, float phi, float kappa_n, float v_out[3], int* out_sat)
{
