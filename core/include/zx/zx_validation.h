...
/* Critical angle for onset of sliding on an inclined plane for dry granular MC.
 * For MC with friction φ and negligible cohesion, θ_c ≈ φ.
 * Returns radians.
 */
ZX_API float ZX_CALL zx_validation_inclined_plane_theta_c(const zx_mc_params* mc);

/* Column collapse runout proxy L/H for aspect ratio ar = H/L0 and friction φ.
 * Provides a monotonic estimate to be refined by empirical fit during calibration.
 */
ZX_API float ZX_CALL zx_validation_column_collapse_runout_ratio(float friction_deg, float aspect_ratio);
