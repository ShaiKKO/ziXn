...
extern "C"
{
   ...
   */
  /* Preconditions: params!=NULL; returns clamped value in [mu_min,mu_max] or a safe default if
   * invalid. */
  ZX_API float ZX_CALL zx_hbp_mu_eff(float gamma_dot, const zx_hbp_params* params, float mu_min,
                                     float mu_max);

  /* Variant with explicit clamp policy and softness parameter (k>0 for smoother clamp). */
  ZX_API float ZX_CALL zx_hbp_mu_eff_policy(float gamma_dot, const zx_hbp_params* params,
                                            float mu_min, float mu_max, zx_mu_clamp_policy policy,
                                            float softness_k);
  ...
  ZX_API void ZX_CALL zx_hbp_update_coarse_grid(zx_tile* pool, uint32_t tile_count, float h,
                                                float dt, const zx_hbp_params* params, float mu_min,
                                                float mu_max);
