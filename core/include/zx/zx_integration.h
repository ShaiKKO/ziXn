...
extern "C"
{
  ...
  typedef struct ZxDambreakParams
  {
    ...
    float k_min;        /* [m^2] */
    float mu_min;
    float mu_max; /* for mu_eff clamp */
    float beta_min;
    float beta_max; /* for beta clamp */
  ...
  /* Compute dam-break metrics on a single-tile coarse grid with HBP diffusion and Darcy drag in
   * bed. */
  ...
  typedef struct ZxBoggingParams
  {
    ...
    float mu_min;
    float mu_max; /* viscosity clamp */
    float permeability;
    float k_min; /* Darcy */
    float beta_min;
    float beta_max;  /* beta clamp */
